function page_ctrl2(data_obj,data) {
    ...
    //翻页容器dom对象,必要参数
    var total_item=data.length;//数据条目总数,默认为0,组件将不加载
    ...
    function page_even() {
        ...
        function change_content(){
            ...
            for(var i = 0;i<total_item;i++)//每页显示的页数。如何调整每页显示的内容刚好对应相应的俄页面
            {
                ...
                pageContent+='<li>';
                pageContent+='<div class="item item1"><p class="Compound-id"><a href="cdetail?id='+data[i].id+'" id="PISMID">'+data[i].id+'</a></p><br><p style="font-size: 18px;color: #000;font-weight: bold">Similarity <span id="MW" style="color: #505050;font-weight: normal">'+toPercent(data[i].value)+'</span></p></div>';

                pageContent+='<div class="item item2"><p><a href="cdetail?id='+data[i].id+'"id="Chemical-name">'+data[i].name+'</a></p><div class="chemical-member"><ul><li><p id="basic">Basic Information</p></li><li><p id="pathway">Targeting Pathway</p></li><li><p id="related">Related Molecules</p></li><li><p id="supporting">Supporting Information</p></li></ul></div></div>';
                pageContent+='<div class="item item3"><ul><li>IUPAC Name <span id="IUPAC-name">'+data[i].basic.iupacName+'</span></li><li>Chemical Formula <span id="Chemical-Formula">'+data[i].basic.chemicalFormular+'</span></li><li>SMILES <span id="SMILES">'+data[i].basic.smiles+'</span></li><li>3D structure file <span id="3D structure file"><a href="./static/jsmol/data/mol2/PIS'+data[i].id+".mol2"+'">'+data[i].id+".mol2"+'</a></span></li><li>Alog P <span id="ClogP">'+data[i].basic.alogP+'</span></li><li>MW <span id="MW">'+data[i].basic.molecularWeight+'</span></li>'+chemitem+'<li>PubChem <span id="PubChem"><a target="_blank" href="https://pubchem.ncbi.nlm.nih.gov/#query='+smileslink+'">'+pubchem+'</a></span></li></ul></div>';
